...
Each of the request verbs share the same configuration method forms. The following examples are for `GET` requests; however, they are representative
of the available configurations. Also, at the end of the User Guide there is a collection of recipe-style example scripts for various common HTTP
operations.
...
==== How many people are in Space? (`get(Class,Closure)`)

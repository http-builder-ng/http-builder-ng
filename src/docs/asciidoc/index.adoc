...
Generally the `HttpBuilder` is configured using one of the static `configure` methods
also show how to create simple inline verb call

Disucss the two factory impls...

These are specific to the configure closure...
...
the following may be defined in the configure closure and shared across multiple calls, or
configured and overwritten by the configure closure of the verbs

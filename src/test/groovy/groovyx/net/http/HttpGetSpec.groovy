...
class HttpGetSpec extends Specification {
    ...
    @Unroll def '[#client] GET /status(#status): verify when handler'() {
        given:
        webServer.dispatcher = new Dispatcher() {
            @Override
            MockResponse dispatch(final RecordedRequest request) throws InterruptedException {
    ...
    private HttpBuilder httpBuilder(final HttpClientType clientType, Closure config = { request.uri = "http://${webServer.hostName}:${webServer.port}" }) {
